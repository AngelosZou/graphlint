# -*- coding: utf-8 -*-
"""C# source parser — tree-sitter-c-sharp based parsing for ``.cs`` files."""
    ...
    def parse_file(self, full_path: str) -> ParseResult:
        ...
        if not _TREE_SITTER_CSHARP_AVAILABLE:
            result.warnings.append(
                _make_warning(
                    "syntax_error", "error",
                    "tree-sitter-c-sharp not installed; install with: "
        ...
        try:
            ...
            # tree-sitter >= 0.24 uses a settable `language` property,
            # older versions use `set_language()`.
        ...
        try:
            # Note: the visitor does not consume namespace_qname; graph.py
            # applies csproj-aware module naming in the parent process.
            module_qname = _file_to_module(file_path)
            import_analyzer = CSharpImportAnalyzer()
            visitor = CSharpVisitor(module_qname, file_path, import_analyzer)
            visitor.visit(tree)

            result.nodes = visitor.nodes
            result.imports = list(visitor.uses)
