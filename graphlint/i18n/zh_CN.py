...
STRINGS: dict[str, str] = {
    ...
    "help.query": "查询依赖关系图",
    "help.build": "构建/重建索引",
    "help.install": "为 Agent 工具安装 graphlint（默认安装 skill；'install dsh' 安装 DeepSeek Harness 插件，'install prompt' 注入配置文件）",
    "help.install.actions": "安装目标",
    "help.install.skill": "将 graphlint SKILL.md 安装到 Agent skill 目录（默认 ~/.agents/skills）（推荐）",
    "help.install.prompt": "将 graphlint 提示词注入 Agent 配置文件",
    "help.install.dsh": "将 dsh-graphlint 插件安装到 DeepSeek Harness profile（DSH 环境推荐 — 工具化集成更稳健）",
    "help.prompt": "将 graphlint 提示词复制到粘贴板",
    "help.uninstall": "移除已安装的 graphlint skill 或提示词",
    "help.uninstall.actions": "卸载目标",
    "help.uninstall.skill": "从 Agent skill 目录移除 graphlint SKILL.md",
    "help.uninstall.prompt": "从 Agent 配置文件中移除注入的 graphlint 提示词",
    "help.config": "管理配置",
    ...
    "help.param.public_as_entry": "将 public 方法（如 Rust pub、C# public、C 外部链接符号）视为执行入口（库模式）",
    ...
    "help.param.install_targets": "skill 目录：agents、claude（逗号分隔）或 all",
    "help.param.dsh_profile": "DeepSeek Harness profile 名称（省略则使用默认 profile）",
    "help.param.dsh_local": "链接本地 integrations/dsh 仓库而非 npm 包（可选路径）",
    # -------- Errors --------
    ...
    "cli.install.skill.installed": "✓ 已安装 graphlint skill v{version} -> {path}",
    "cli.install.skill.updated": "✓ 已将 graphlint skill 从 v{old} 更新到 v{new}：{path}",
    "cli.install.skill.uptodate": "- graphlint skill 已是最新版本：{path}",
    "cli.install.skill.failed": "✗ 写入失败：{path}",
    "cli.install.dsh.not_found": "未在 PATH 中找到 dsh CLI。请先安装 DeepSeek Harness 后重试。",
    "cli.install.dsh.local_missing": "在 {path} 未找到本地 integrations/dsh。请在 graphlint 仓库根目录运行，或使用 --local PATH 指定路径。",
    "cli.install.dsh.done": "✓ 已将 dsh-graphlint 插件添加到 profile '{profile}'。请重启 dsh web 并刷新浏览器页面。",
    "cli.install.dsh.failed": "✗ dsh 插件安装失败。",
    "cli.uninstall.select_prompt": "输入要卸载的编号（逗号分隔）、all 或留空取消：",
    "cli.uninstall.skill.removed": "✓ 已从 {path} 移除 graphlint skill",
    "cli.uninstall.skill.not_found": "- 在 {path} 未找到 graphlint skill",
    "cli.uninstall.skill.foreign": "- {path} 已存在但不是 graphlint 安装的，已跳过",
