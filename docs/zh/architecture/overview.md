...
│    • 连通分量分析 — 对组装的完整图执行可达性计算               │
├─────────────────────────────────────────────────────────────┤
│  阶段 5: 持久化（Persist）                                    │
│  将更新的节点和边写回 SQLite。修复未变更节点的 parent_node_id  │
│  重映射：当父节点在变更文件中获得新 ID 时，子节点在内存中的    │
│  重映射原先未被写回数据库。                                   │
├─────────────────────────────────────────────────────────────┤
│  阶段 6: 验证（Verify）                                       │
│  轻量级一致性检查：检测悬挂边引用并记录警告，不阻断构建。      │
└─────────────────────────────────────────────────────────────┘
```
...
- **可达性传播**：上游边重建后，`GraphBuilder` 对组装的完整图执行连通分量分析，根据新的上游连接判断哪些节点从入口点可达。当节点的可达性发生变化（上游引用被移除可能导致之前可达的节点变为不可达，反之亦然），该变化会通过复原的下游边传播到下游节点。

- **缓存策略**：未变更文件不需要重新解析。其节点、边和元数据直接从 SQLite 数据库加载，通过 SHA256 哈希比较验证。这使得增量重建的成本与变更文件数量成正比，而非整个代码库的规模。
