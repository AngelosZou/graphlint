...
test('configArgv: show takes no key', () => {
  assert.deepEqual(configArgv(EXE, 'show'), [EXE, 'config', '--lang', 'en', 'show'])
})

test('configArgv: get passes the key', () => {
  assert.deepEqual(configArgv(EXE, 'get', 'lang'), [EXE, 'config', '--lang', 'en', 'get', '--key', 'lang'])
})

test('configArgv: set passes key and value', () => {
  assert.deepEqual(configArgv(EXE, 'set', 'lang', 'en'), [EXE, 'config', '--lang', 'en', 'set', '--key', 'lang', '--value', 'en'])
})
