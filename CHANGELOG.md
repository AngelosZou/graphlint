...
### Changed
- Pre-built entry indexes (`nid_to_entries`, `fid_to_entries`) for O(1)
  per-component entry matching, eliminating repeated linear scans in
  `_match_entries` during connected component analysis
- Connected component BFS now collects reachable node seeds in a single
  traversal pass instead of a post-hoc set comprehension
- Dead-code component merge loop replaced with BFS-based group merging,
  eliminating repeated list scans and improving large-codebase performance
